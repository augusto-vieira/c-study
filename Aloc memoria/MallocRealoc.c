...
/*
	utilização das funções malloc e realloc para criar e aumentar
	o tamanho de um vetor dinamicamente (em tempo de execução). No caso de
	algum erro, as funções retornam um ponteiro nulo (NULL) para indicar erro de
	alocação de memória.
*/
...
int main(void){
	...
	*/
	...
	for(k = 0; k < i; k++){
		printf("Digite o numero para indice %d -> ",k);
		scanf("%d",&p[k]);
	}
